...
public sealed class MacroLibraryServiceTests
{
    ...
    [TestMethod]
    public async Task LoadAsync_DuplicateUuid_ReassignsBothMacrosAndPreservesBothFiles()
    {
        using var directory = new TemporaryDirectory();
        var paths = new MacroDataPaths(directory.Path);
        var service = new MacroLibraryService(paths);
        ...
        result.Macros.Select(item => item.Name).Should().BeEquivalentTo(["一", "二"]);
        result.Macros.Select(item => item.Id).Should().OnlyHaveUniqueItems();
        result.Macros.Should().OnlyContain(item => item.Id != macro.Id);
        result
            .Errors.Should()
            .ContainSingle()
            .Which.Should()
            .Contain("一 与 二 的 ID 重复")
            .And.Contain("请重新绑定");
        ...
        reloaded
            .Macros.Select(item => item.Id)
            .Should()
            .BeEquivalentTo(result.Macros.Select(item => item.Id));
    }
