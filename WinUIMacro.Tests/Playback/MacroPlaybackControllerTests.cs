...
public sealed class MacroPlaybackControllerTests
{
    ...
    [TestMethod]
    public void UpdateConfiguration_InvalidBoundMacro_FailsBeforeTrigger()
    {
        using var controller = new MacroPlaybackController(_ => { });
        var macro = new MacroDefinition(
            Guid.NewGuid(),
            "invalid",
            MacroPlaybackMode.Once,
            [new MacroNode(MacroNodeType.Delay, "-1")]
        );

        var update = () =>
            controller.UpdateConfiguration([macro], [new MacroTriggerBinding("A", macro.Id)]);

        update.Should().Throw<InvalidDataException>();
    }

    [TestMethod]
    public void UpdateConfiguration_InvalidUnboundMacro_DoesNotCompile()
    {
        using var controller = new MacroPlaybackController(_ => { });
        var macro = new MacroDefinition(
            Guid.NewGuid(),
        ...
        var update = () => controller.UpdateConfiguration([macro], []);

        update.Should().NotThrow();
    }

    [TestMethod]
    public async Task UpdateConfiguration_FailedCompilation_PreservesPreviousPreparedMacro()
    {
        ConcurrentQueue<InputOperation> operations = new();
        using var controller = CreateController(operations.Enqueue, MacroPlaybackMode.Once);
        ...
        var update = () =>
            controller.UpdateConfiguration([invalid], [new MacroTriggerBinding("A", invalid.Id)]);
        update.Should().Throw<InvalidDataException>();
        controller.ProcessInput(KeyA(true));
        await WaitUntilAsync(() => operations.Count >= 2);
        await controller.StopAsync();
